...
import { validateSchemaPartially } from '../../../../shared/lib/validation';
import { useRegisterUser } from '../api/useAuthenticationMutations';
...
export const RegistrationProvider: React.FC<RegistrationProviderProps> = ({
  ...
  const steps = ['Date of birth', 'Identity', 'Identifier', 'Password'];
  ...
  const goToNextScreen = useCallback(() => {
    const stepRoutes: Record<number, Href> = {
      0: '/(auth)/register/identity',
      1: '/(auth)/register/mobile',
      2: '/(auth)/register/security',
    };
